...
#prep EMU data
fn = "/Users/robertadavidson/Box Sync/Robbi_PhD/09_Arbor_Bias_Chapter/worldPCA_1.eigenvecs"
emucDat1 = read.table(fn, col.names=c("PC1", "PC2", "PC3", "PC4", "PC5", "PC6", "PC7", "PC8", "PC9", "PC10")) #emu output
ind = "/Users/robertadavidson/Box Sync/Robbi_PhD/09_Arbor_Bias_Chapter/worldPCA_1.ind"
...
write_tsv(anc, "/Users/robertadavidson/Box Sync/Robbi_PhD/09_Arbor_Bias_Chapter/AP_ancient_all_1.ID.txt")

#prep smartPCA data
fn = "/Users/robertadavidson/Box Sync/Robbi_PhD/09_Arbor_Bias_Chapter/worldPCA_1.outlierremoval.pca.evec.txt"
...
## plot
PC1_2_regionemu <- ggplot() + #each species is represented by a different shape
  geom_point(data=modernemu, aes(PC1,PC2), colour="grey") +
  geom_point(data=ancientemu, aes(PC1,PC2, fill = Populations), shape = 21, size = 4, alpha=1, stroke = 0.5) + #customised scatter plot 
 ...
 # geom_text_repel(data = modern, aes(label=__Political_Entity, y = PC1, x =PC2), segment.size  = 0.2, 
...
#ggplot 1v2
PC1_2_typeemu <- ggplot() + #each species is represented by a different shape
  geom_point(data=modernemu, aes(PC1,PC2), colour="grey") +
  geom_point(data=ancientemu, aes(PC1,PC2, fill = Type,colour=Type, shape = Type), size = 4, alpha=1, stroke = 0.3) + #customised scatter plot 
  guides(fill=guide_legend(ncol =1)) + guides(colour=guide_legend(ncol =1)) + guides(shape=guide_legend(ncol =1)) + 
  ...
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(), #remove minor gridline
        axis.title.x.top = element_blank(), axis.text.x.top = element_blank(), #remove labels on top x-axis
        axis.title.y.right = element_blank(), axis.text.y.right = element_blank(), legend.text = element_markdown(),
        axis.ticks = element_blank(), legend.position = c(0.8,0.8))  #remove labels on right y-axis
PC1_2_typeemu #show plot

#ggplotly(PC1_2_type)
## plot
PC1_2_regionPCA <- ggplot() + #each species is represented by a different shape
  geom_point(data=modernPCA, aes(PC1,-PC2), colour="grey") +
  geom_point(data=ancientPCA, aes(PC1,-PC2, fill = Populations), shape = 21, size = 4, alpha=1, stroke = 0.5) + #customised scatter plot 
  guides(fill=guide_legend(ncol =1)) + guides(colour=guide_legend(ncol =1)) + guides(shape=guide_legend(ncol =1)) + 
  scale_fill_manual(name="Population",labels=labels, values = colours34) +
  # geom_text_repel(data = modern, aes(label=__Political_Entity, y = PC1, x =PC2), segment.size  = 0.2, 
  #                 segment.color = "grey80", colour="black", force=4, show.legend = FALSE, max.overlaps = Inf, size=2) +
...
#ggplot 1v2
PC1_2_typePCA <- ggplot() + #each species is represented by a different shape
  geom_point(data=modernPCA, aes(PC1,-PC2), colour="grey") +
  geom_point(data=ancientPCA, aes(PC1,-PC2, fill = Type, colour = Type, shape = Type), size = 4, alpha=1, stroke = 0.3) + #customised scatter plot 
  guides(fill=guide_legend(ncol =1)) + guides(colour=guide_legend(ncol =1)) + guides(shape=guide_legend(ncol =1)) + 
